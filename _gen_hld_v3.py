...
def build_document():
    ...
    meta.add_run('Version 3.0  |  May 2026\n').font.size = Pt(12)
    ...
    add_bullet(doc, 'Resize a Server wizard — analyze 30-day CPU/memory usage, compare alternatives, one-click resize with downtime warning')
    add_bullet(doc, 'Optimize a Cluster wizard — 7-check ASG health report (multi-AZ, Spot mix, LB, scaling, diversification) with grade A-D')
    add_bullet(doc, 'Spot Instance Management — configure ASG Spot mix, EventBridge push notifications, savings ledger for gainshare billing')
    add_bullet(doc, 'FinOps Settings Healthcheck — scored audit of 9 best-practice settings per account')
    add_bullet(doc, 'Automated Scheduler — EventBridge-based stop/start schedules for EC2/RDS with cost savings tracking')
    add_bullet(doc, 'Free Tier awareness — real-time Free Tier usage via freetier:GetFreeTierUsage API')
    ...
    components = [
        ...
        ('API', 'API Gateway HTTP', 'ViewMyBill-API (l2fd4h481h)', 'us-east-1'),
        ('Bill Analyzer', 'Lambda', 'aws-bill-analyzer-viewmybill', 'us-east-1'),
        ('Upload Handler', 'Lambda', 'aws-bill-analyzer-upload-handler', 'us-east-1'),
        ('OTP Handler', 'Lambda', 'aws-bill-analyzer-otp-handler', 'us-east-1'),
        ('Member Handler', 'Lambda', 'aws-bill-analyzer-member-api (512 MB, 120s)', 'us-east-1'),
        ('Admin Handler', 'Lambda', 'aws-bill-analyzer-admin-api', 'us-east-1'),
        ('Agent Action', 'Lambda', 'aws-bill-analyzer-agent-action', 'us-east-1'),
        ('Scheduler Executor', 'Lambda', 'slashmybill-scheduler-executor', 'us-east-1'),
        ('AI Model', 'Amazon Bedrock', 'us.amazon.nova-2-lite-v1:0 (cross-region)', 'us-east-1'),
        ('Bedrock Agent', 'Amazon Bedrock Agents', 'IDG5VJGUOZ5W / Alias 9VYFXAEEH6', 'us-east-1'),
        ('User Pool', 'Cognito', 'us-east-1_FlR2CmFu0 (SlashMyBill-Members)', 'us-east-1'),
        ('Email', 'SES', 'noreply@slashmycloudbill.com', 'us-east-1'),
        ('TLS Certs', 'ACM', '*.slashmycloudbill.com, *.eshkolai.com', 'us-east-1'),
        ('Spot SNS', 'SNS', 'SlashMyBill-SpotInterruptions', 'us-east-1'),
        ('Spot Ledger', 'DynamoDB', 'SpotSavingsLedger', 'us-east-1'),
        ('CI/CD', 'GitHub Actions', 'OIDC → GitHubActionsRole', 'us-east-1'),
    ]
    ...
    tabs = [
        ('Configure', 'Account Management',
         'Add/remove AWS accounts, IAM role setup wizard, CloudFormation template download, FinOps Settings healthcheck, Tag Policy, Spot Management'),
        ('Observe', 'Dashboard & Monitoring',
         '8 customizable widgets, multi-account selector, Live Business Metrics, FinOps Settings score, auto-refresh with 5-min cache'),
        ('Chat', 'AI-Powered Console',
         'Natural language queries, Bedrock Agent mode, conversation history, feedback (thumbs up/down), token tracking, navigation links to Act'),
        ('Act', 'Cleanup & Optimization',
         'Waste Cleanup (7 categories), Optimize (Resize Server + Optimize Cluster + Spot + Scan for Savings), Scheduler (EventBridge stop/start)'),
    ]
    ...
    tables_data = [
        ...
        ('ViewMyBill-OTP', 'email', 'None',
         ...
         'ttl (5 min)', 'None'),
        ('ViewMyBill-CostOptimizationTips', 'service', 'tipId',
         'category, title, description, severity, serviceKey, checkImplemented, actionType, actionLabel, level',
         'None', 'None'),
        ('MemberPortal-Members', 'email', 'None',
         'name, tier, tokens, cognito_sub, spotConfig, lastScan, tagPolicy',
         'None', 'None'),
        ('MemberPortal-Accounts', 'memberEmail', 'accountId',
         'accountName, connectionStatus, sortOrder, addedAt',
         'None', 'None'),
        ('MemberPortal-AgentFeedback', 'memberEmail', 'interactionId',
         'query, response, rating, comment, model, timestamp',
         'None', 'None'),
        ('MemberPortal-BusinessMetrics', 'memberEmail', 'metricMonth',
         'metrics JSON, computedAt',
         'None', 'None'),
        ('SpotSavingsLedger', 'pk (email#accountId)', 'sk (timestamp#instanceId)',
         'instanceType, onDemandRate, spotRate, savingsPerHour, gainshareAmount, eventType, asgName',
         'ttl (12 months)', 'MemberTimeIndex (memberEmail, recordedAt)'),
    ]
    ...
    # ════════════════════════════════════════════════════════════════════════
    # 15. OPTIMIZE TAB — NEW FEATURES (v3)
    # ════════════════════════════════════════════════════════════════════════
    doc.add_heading('15. Optimize Tab — New Features (v3)', level=1)

    doc.add_heading('15.1 Resize a Server Wizard', level=2)
    add_body(doc,
        'The Resize a Server wizard (Act > Optimize) guides users through analyzing and '
        'resizing EC2 instances to cheaper types with one click.')
    add_bullet(doc, 'Step 1: Select account and EC2 instance (loaded via /members/servers/list-instances)')
    add_bullet(doc, 'Step 2: Click "Optimize" — analyzes 30 days of CloudWatch CPU and memory metrics')
    add_bullet(doc, 'Step 3: Full instance spec card — vCPU, memory, network, EBS IOPS, architecture, hypervisor, Free Tier status')
    add_bullet(doc, 'Step 4: Sortable alternatives table — 35+ instance types from catalog, filtered by architecture compatibility')
    add_bullet(doc, 'Step 5: Click "Resize" — stop instance → modify type → start instance (1-3 min downtime)')
    add_bullet(doc, 'Architecture safety: DescribeInstanceTypes validates x86/ARM compatibility before showing alternatives')
    add_bullet(doc, 'Free Tier integration: freetier:GetFreeTierUsage shows current Free Tier usage per service')
    add_bullet(doc, 'Email notification: SES confirmation with before/after cost comparison')
    add_body(doc, 'API endpoints: POST /members/servers/analyze, POST /members/servers/resize, POST /members/servers/list-instances')

    doc.add_heading('15.2 Optimize a Cluster Wizard', level=2)
    add_body(doc,
        'The Optimize a Cluster wizard (Act > Optimize) analyzes existing Auto Scaling Groups '
        'against 7 best practices and returns a health report with grade A-D.')

    tbl = doc.add_table(rows=1, cols=4)
    tbl.style = 'Table Grid'
    hdr = tbl.rows[0].cells
    for i, h in enumerate(['Check', 'API Used', 'Pass Condition', 'Fix Recommendation']):
        hdr[i].text = h
        set_cell_shading(hdr[i], "2E4057")
        for p in hdr[i].paragraphs:
            for run in p.runs:
    ...
    checks = [
        ('Multi-AZ', 'DescribeAutoScalingGroups', 'AvailabilityZones >= 2', 'Add subnets from additional AZs'),
        ('Load Balancer', 'DescribeTargetHealth', 'TargetGroupARNs not empty', 'Create ALB + attach target group'),
        ('Spot Mix', 'DescribeAutoScalingGroups', 'MixedInstancesPolicy with price-capacity-optimized', 'Enable MixedInstancesPolicy'),
        ('Diversification', 'DescribeInstances', 'Instance types in use >= 2', 'Use attribute-based instance selection'),
        ('Scaling Policy', 'DescribePolicies', 'ScalingPolicies not empty', 'Add target tracking policy (CPU 60%)'),
        ('Launch Template', 'DescribeAutoScalingGroups', 'LaunchTemplate set (not LaunchConfiguration)', 'Migrate to Launch Template'),
        ('Health Check Type', 'DescribeAutoScalingGroups', 'HealthCheckType = ELB when LB attached', 'Change health check type to ELB'),
    ]
    for row_data in checks:
        add_table_row(tbl, row_data)

    add_body(doc, 'API endpoint: POST /members/cluster/analyze')

    doc.add_heading('15.3 Spot Instance Management', level=2)
    add_body(doc,
        'The Spot Instance Management feature enables ASG Spot optimization with push-based '
        'interruption notifications via EventBridge → SNS → Lambda → SES.')

    add_bullet(doc, 'Configure: Enable Spot management per account, qualify/exclude ASGs, deploy EventBridge rule')
    add_bullet(doc, 'Plan: Configure capacity mix (On-Demand base + Spot %), vCPU/memory range')
    add_bullet(doc, 'Migrate: Dry-run → Execute (MixedInstancesPolicy with price-capacity-optimized) → Rollback (7-day window)')
    add_bullet(doc, 'Interruption pipeline: EventBridge rule in customer account → SNS SlashMyBill-SpotInterruptions → Member Handler → SES email')
    add_bullet(doc, 'Savings Ledger: SpotSavingsLedger DynamoDB table tracks per-instance-hour savings for 30% gainshare billing')
    add_bullet(doc, 'Email notifications: migration complete, interruption detected (with ASG auto-replacement confirmation), rollback complete')
    add_body(doc, 'API endpoints: POST /members/spot/config, POST /members/spot/qualify, POST /members/spot/plan, POST /members/spot/migrate, GET /members/spot/dashboard')

    doc.add_heading('15.4 FinOps Settings Healthcheck', level=2)
    add_body(doc,
        'The FinOps Settings Healthcheck (Configure > FinOps Settings) audits each connected '
        'account against 9 best-practice settings, split into two groups:')
    add_bullet(doc, 'Configurable via SlashMyBill (scored): Cost Allocation Tags, Anomaly Detection, Budgets, Tag Coverage, Hourly Granularity')
    add_bullet(doc, 'Requires AWS Console (informational): Trusted Advisor, Compute Optimizer, Savings Plans, Reserved Instances')
    add_bullet(doc, 'Score: 0-100 based on configurable checks only')
    add_bullet(doc, 'Fix actions: one-click remediation for supported checks (e.g., activate cost allocation tags)')
    add_body(doc, 'API endpoint: POST /members/healthcheck/scan, POST /members/healthcheck/fix')

    doc.add_heading('15.5 Automated Scheduler', level=2)
    add_body(doc,
        'The Scheduler (Act > Scheduler) creates EventBridge Scheduler rules to automatically '
        'stop and start EC2 instances and RDS databases on a schedule.')
    add_bullet(doc, 'Analyze: Scans accounts for scheduling candidates (non-prod instances, low CPU)')
    add_bullet(doc, 'Create: EventBridge Scheduler rule with cron expression, target Lambda (slashmybill-scheduler-executor)')
    add_bullet(doc, 'Executor Lambda: Assumes cross-account role, stops/starts instances, logs results')
    add_bullet(doc, 'Savings estimate: based on hours saved per week × instance hourly rate')
    add_bullet(doc, 'Pause/Resume/Delete: full lifecycle management via API')
    add_body(doc, 'API endpoints: POST /members/schedules/analyze, POST /members/schedules/create, GET /members/schedules, PUT /members/schedules/status')

    doc.add_heading('15.6 Tag Policy', level=2)
    add_body(doc,
        'The Tag Policy (Configure > Tag Policy) allows members to define required tag keys '
        'for their organization. The waste scan uses these keys to identify untagged resources.')
    add_bullet(doc, 'Default keys: Environment, Owner, CostCenter, Application')
    add_bullet(doc, 'Stored per member in MemberPortal-Members.tagPolicy field')
    add_bullet(doc, 'Tag scan: ResourceGroupsTaggingAPI scans all resources, reports coverage %')
    add_bullet(doc, 'Tag apply: bulk-apply tags to selected resources via cross-account role')
    add_body(doc, 'API endpoints: GET /members/tag-policy, POST /members/tag-policy, POST /members/tags/scan, POST /members/tags/apply')

    # ════════════════════════════════════════════════════════════════════════
    # 16. BEDROCK AGENT (v3)
    # ════════════════════════════════════════════════════════════════════════
    doc.add_heading('16. Bedrock Agent', level=1)

    add_body(doc,
        'The Bedrock Agent (IDG5VJGUOZ5W, Alias 9VYFXAEEH6) provides autonomous FinOps '
        'analysis via 13 action tools. The agent is deployed in us-east-1 and called '
        'cross-region from the Member Handler.')

    tbl = doc.add_table(rows=1, cols=3)
    tbl.style = 'Table Grid'
    hdr = tbl.rows[0].cells
    for i, h in enumerate(['Tool', 'API Path', 'Purpose']):
        hdr[i].text = h
        set_cell_shading(hdr[i], "2E4057")
        for p in hdr[i].paragraphs:
            for run in p.runs:
    ...
    tools = [
        ('getCostData', '/get-cost-data', 'Cost breakdown by service (30 days) + daily trend'),
        ('getMonthlyComparison', '/get-monthly-comparison', 'Month-over-month cost comparison (3-6 months)'),
        ('getEC2Instances', '/get-ec2-instances', 'EC2 instance inventory with state and type'),
        ('getRDSInstances', '/get-rds-instances', 'RDS instance inventory with engine and size'),
        ('getLambdaFunctions', '/get-lambda-functions', 'Lambda function list with memory and runtime'),
        ('getS3Buckets', '/get-s3-buckets', 'S3 bucket list with size and storage class'),
        ('getEBSVolumes', '/get-ebs-volumes', 'EBS volume inventory with type and attachment'),
        ('getNetworkResources', '/get-network-resources', 'EIPs, NAT Gateways, VPC endpoints'),
        ('getBudgets', '/get-budgets', 'Budget list with current spend vs limit'),
        ('getFinOpsSettings', '/get-finops-settings', 'FinOps settings healthcheck results'),
        ('getOptimizationTips', '/get-optimization-tips', 'Knowledge base tips filtered by service'),
        ('getAWSPricing', '/get-aws-pricing', 'Real-time on-demand pricing from Pricing API'),
        ('getSpotPlacementScore', '/get-spot-placement-score', 'Spot capacity availability scores by region/AZ'),
    ]
    for row_data in tools:
        add_table_row(tbl, row_data)

    add_body(doc,
        'Agent instructions are stored as code in agent-action/agent-instructions.md and '
        'deployed on every push. The agent is updated via infrastructure/update-bedrock-agent.py '
        'as part of the CI/CD pipeline.')

    # ════════════════════════════════════════════════════════════════════════
    # COMMITTED DISCOUNT ANALYZER & SAVINGS EXPLORER
    # ════════════════════════════════════════════════════════════════════════
    doc.add_heading('16. Committed Discount Analyzer', level=1)

    add_body(doc,
        'The Committed Discounts section in the Act tab analyzes Reserved Instance (RI) and '
        'Savings Plan (SP) coverage, utilization, and purchase recommendations using AWS Cost '
        'Explorer APIs via the existing cross-account STS AssumeRole pattern.')

    doc.add_heading('16.1 API Routes', level=2)
    add_bullet(doc, 'POST /members/committed-discounts/scan — Full analysis: coverage, utilization, SP/RI recommendations, P10 baseline, expiring commitments, laddering strategy')
    add_bullet(doc, 'POST /members/committed-discounts/ladder — Custom laddering strategy with user-specified monthly commitment')

    doc.add_heading('16.2 Backend Modules', level=2)
    add_bullet(doc, 'P10 Baseline Calculator — 10th percentile of hourly compute spend (safest commitment level)')
    add_bullet(doc, 'SP Recommendation Retriever — Queries GetSavingsPlansPurchaseRecommendation for Compute SP and EC2 Instance SP across all term/payment combos')
    add_bullet(doc, 'RI Recommendation Retriever — Queries GetReservationPurchaseRecommendation for EC2 and RDS (Standard + Convertible, all terms/payments)')
    add_bullet(doc, 'Coverage/Utilization Scanner — Aggregates SP and RI coverage/utilization with per-service breakdown')
    add_bullet(doc, 'Expiring Commitments Tracker — Identifies SPs and RIs expiring within 90 days')
    add_bullet(doc, 'Database SP Threshold — Includes Database SP recommendations when RDS+ElastiCache spend > $50/month')
    add_bullet(doc, 'Laddering Strategy Generator — Divides commitment into 4 staggered purchases (3 months apart)')

    doc.add_heading('16.3 Commitment Savings Explorer (Frontend)', level=2)
    add_body(doc,
        'Interactive exploration of SP and RI options using cached scan data from sessionStorage:')
    add_bullet(doc, 'SP Explorer — Select term (1yr/3yr) and payment (All Upfront/Partial/No Upfront), view savings/hr, savings/month, break-even, "Best Value" badge')
    add_bullet(doc, 'RI Explorer — Select instance type, offering class (Standard/Convertible), term, payment, view TCO, break-even, "Lowest TCO" badge')
    add_bullet(doc, 'Compare All Options — Toggle table showing all combinations side-by-side with highlighting')
    add_bullet(doc, 'Laddering Timeline — Visual progress bar with 4 purchase milestones, monthly amounts, preset buttons (Conservative/Moderate/Aggressive)')

    doc.add_heading('16.4 Dashboard Integration', level=2)
    add_bullet(doc, 'SP Coverage Widget — Progress bar with coverage % (amber <50%) and utilization % (red <80%)')
    add_bullet(doc, 'RI Coverage Widget — Progress bar with coverage %, utilization %, underutilized count badge')
    add_bullet(doc, 'KPI Bar — "Commitment Savings (estimated)" line in Potential Savings card')

    doc.add_heading('16.5 Chat Integration', level=2)
    add_bullet(doc, 'Savings comparison bar charts injected when AI discusses commitments')
    add_bullet(doc, '"Go to Act → Committed Discounts" navigation links in chat responses')

    doc.add_heading('16.6 Key Design Decisions', level=2)
    add_bullet(doc, 'P10 baseline over average — uses 10th percentile of hourly spend to avoid over-commitment during usage dips')
    add_bullet(doc, 'Daily-to-hourly conversion — when hourly granularity unavailable, divides daily values by 24')
    add_bullet(doc, 'Safe commitment range — 60-70% of average for stable usage, P10 floor for variable usage')
    add_bullet(doc, 'Frontend caching — scan results cached in sessionStorage, no repeated API calls when navigating')
    add_bullet(doc, 'Permission pre-check — lightweight 1-day coverage call before full scan to validate IAM permissions')
    add_bullet(doc, 'Parallel execution — ThreadPoolExecutor for concurrent Cost Explorer API calls within 30s timeout')

    # ── Save ────────────────────────────────────────────────────────────────
    doc.save('SlashMyBill-HLD-v3.docx')
    print('SlashMyBill-HLD-v3.docx generated successfully!')
