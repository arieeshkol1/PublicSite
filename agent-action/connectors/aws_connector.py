...
class AWSConnector(CloudConnector):
    ...
    def get_compute_instances(self, account_id: str, member_email: str, params: dict) -> dict:
        ...
        Calls the provider Pricing API dynamically for hourly rates (no hardcoded prices).
        """
        try:
            creds = self._assume_role(account_id, member_email)
            ...
            return {
                'instances': instances,
                'count': len(instances),
                'note': 'Cost shown is per-instance on-demand rate (compute only, excludes storage/network). Use getCostBreakdown for total account costs.',
            }
        except Exception as e:
            return {'error': str(e)}
    ...
    def _lookup_instance_pricing(self, instance_types: set, region: str) -> dict:
        ...
        try:
            ...
            logger.warning(f"Pricing API unavailable: {e}")

        # Fill None for types not found
        for itype in instance_types:
