...
def _extract_text_and_tables(pdf_bytes: bytes, text_only: bool = False) -> Dict[str, Any]:
    ...
    try:
        ...
        if total_pages == 0:
            raise ValueError("The PDF file has no pages. Please upload a valid AWS invoice PDF.")
        ...
        if not text_only:
            logger.info("STEP 4.2.2b: Opening PDF with pdfplumber for table extraction")
            _phase2_start = _time.time()
            pdf_stream2 = io.BytesIO(pdf_bytes)
            with pdfplumber.open(pdf_stream2) as pdf:
                for i, page in enumerate(pdf.pages[:pages_to_process]):
                    page_text = all_text_parts[i] if i < len(all_text_parts) else ""

                    # Skip tables from our own analysis report pages
                    if any(marker in page_text[:200] for marker in _REPORT_MARKERS):
                        logger.info("STEP 4.2.2b: Skipping report page %d", i + 1)
                        continue
                    ...
                    logger.info("STEP 4.2.2b: Page %d/%d tables extracted (%.1fs, %d tables)", i + 1, pages_to_process, _time.time() - _tbl_start, len(page_tables))

            logger.info("STEP 4.2.2b: pdfplumber table extraction done in %.1fs, %d total tables", _time.time() - _phase2_start, len(all_tables))

        logger.info("STEP 4.2.2: Extraction complete. %d text parts, %d tables", len(all_text_parts), len(all_tables))
        return {
            "text": "\n".join(all_text_parts),
            "tables": all_tables,
        }
