...
def _extract_text_and_tables(pdf_bytes: bytes, text_only: bool = False) -> Dict[str, Any]:
    ...
    try:
        logger.info("STEP 4.2.2a: Opening PDF (%d bytes) with pdfplumber", len(pdf_bytes))
        pdf_stream = io.BytesIO(pdf_bytes)
        with pdfplumber.open(pdf_stream) as pdf:
            logger.info("STEP 4.2.2a: PDF opened successfully")
            if len(pdf.pages) == 0:
                raise ValueError(
            ...
            for i, page in enumerate(pdf.pages[:pages_to_process]):
                ...
                logger.info("STEP 4.2.2a: Page %d/%d text extracted (%.1fs)", i + 1, pages_to_process, _time.time() - _page_start)

                if not text_only:
                    # Skip tables from our own analysis report pages
                    if any(marker in page_text[:200] for marker in _REPORT_MARKERS):
                        logger.info("STEP 4.2.2a: Skipping report page %d", i + 1)
                        continue

                    logger.info("STEP 4.2.2a: Extracting tables from page %d/%d", i + 1, pages_to_process)
            ...
            logger.info("STEP 4.2.2a: Extraction complete. %d text parts, %d tables", len(all_text_parts), len(all_tables))
            return {
                "text": "\n".join(all_text_parts),
                "tables": all_tables,
            }
