...
async function loadLeads(){try{showLoading();var data=await apiRequest('GET','/admin/leads');allLeads=data.leads||[];renderLeads(allLeads);}catch(e){showNotification('Failed to load leads.','error');}finally{hideLoading();}}
...
async function loadTips(){try{showLoading();var data=await apiRequest('GET','/admin/tips');allTips=data.tips||[];renderTips(allTips);}catch(e){showNotification('Failed to load tips.','error');}finally{hideLoading();}}
...
async function saveTip(){tipFormError.textContent='';var tipData={};for(var i=0;i<TIP_FIELDS.length;i++){var f=TIP_FIELDS[i],val=document.getElementById('tip-'+f).value.trim();if(!val){tipFormError.textContent='All fields are required.';return;}tipData[f]=val;}try{showLoading();if(editingTip){await apiRequest('PUT','/admin/tips',tipData);showNotification('Tip updated.','success');}else{await apiRequest('POST','/admin/tips',tipData);showNotification('Tip created.','success');}hideTipForm();await loadTips();}catch(e){if(e.status===409)tipFormError.textContent='A tip with this service and ID already exists.';else tipFormError.textContent=e.message||'Failed to save tip.';}finally{hideLoading();}}
...
async function deleteTip(){if(!deletingTip)return;var svc=deletingTip.service,tid=deletingTip.tipId;hideDeleteDialog();try{showLoading();await apiRequest('DELETE','/admin/tips',{service:svc,tipId:tid});showNotification('Tip deleted.','success');await loadTips();}catch(e){if(e.status===404){showNotification('Tip not found.','error');await loadTips();}else showNotification(e.message||'Failed to delete tip.','error');}finally{hideLoading();}}
